public class ThrowWithAirResistance extends Throw {

    protected double x0; // Start width

    protected static final double PAIR = 1.293; // Density of air (kg/m³)

    ThrowWithAirResistance(double y0, double v0, double alpha) {
        super(y0, v0, alpha);
    }

    public double[][] calculateParabel(double dt, double vW, double cW, double r, double m) {
        ...
        for (int i = 0; i < result[0].length && result[2][i] > 0; i++) {
            ...
            if (result[0][j] > gettMax()) tMax = result[0][j];
            if (result[1][j] > getxMax()) xMax = result[1][j];
            if (result[2][j] > getyMax()) yMax = result[2][j];
        }
